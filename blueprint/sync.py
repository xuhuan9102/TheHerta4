...
def find_nodes_by_object_id(tree, object_id):
    ...
    for node in tree.nodes:
        if node.bl_idname == 'SSMTNode_Object_Info':
            if getattr(node, 'object_id', '') == object_id:
                nodes.append(node)
    return nodes
...
def find_nodes_by_object_name(tree, object_name):
    ...
    for node in tree.nodes:
        if node.bl_idname == 'SSMTNode_Object_Info':
            if getattr(node, 'object_name', '') == object_name:
                nodes.append(node)
    return nodes
...
def sync_nodes_to_objects(nodes, context):
    ...
    for node in nodes:
        ...
        if obj and obj not in objects_to_select:
...
def check_node_selection():
    ...
    object_info_nodes = [n for n in selected_nodes if n.bl_idname == 'SSMTNode_Object_Info']
    if object_info_nodes:
        sync_nodes_to_objects(object_info_nodes, context)
        ...
        if tree.bl_idname == 'SSMTBlueprintTreeType':
            for node in tree.nodes:
                if node.bl_idname == 'SSMTNode_Object_Info':
                    ...
                    if node_object_id == object_id and node_object_name != new_name:
...
class SSMT_OT_SyncSelectedNodeToObject(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        selected_nodes = [n for n in tree.nodes if n.select and n.bl_idname == 'SSMTNode_Object_Info']
        if not selected_nodes:
            self.report({'WARNING'}, "请选择至少一个Object Info节点")
            return {'CANCELLED'}
