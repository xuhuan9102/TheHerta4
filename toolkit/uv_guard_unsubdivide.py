...
两条**用户裁定为预期行为**（2026-09-15，勿当 bug 去"修"）：
1. **用户标的缝合线（use_seam）会被清掉**：反细分要重建拓扑，被溶解的原边消失、
   幸存边都是新边，缝标记随之丢失。这是预期结果，不做备份/还原。
2. **多物体编辑下不做额外遍历**：反细分只处理当前活动物体；细分沿用
   bpy.ops.mesh.subdivide 的原生作用范围（编辑模式内所有对象），而边界多边面
   三角化只处理活动物体。同样按预期行为处理。
